...
# @router.get("/poll")
# def poll():
#     return {"message": "You shall be called the son.\nOf the most high"}

@router.get("/", response_model=UserSchema)
async def get_user(session: Session= Depends(get_db),
                user: User = Depends(get_current_user)
                ):
    # user:User = session.query(User).filter_by(public_id=id).one_or_none()
    # if not user:
    #     raise HTTPException(404, detail=f"No User with ID '{id}' found")
    return user
...
async def create_user(form:UserForm = Depends(UserForm), session:Session =Depends(get_db)):
...
async def update_user(username: Optional[str] = Form(None, min_length=3),
    ...
    # user:User = session.query(User).filter_by(public_id=id).one_or_none()
    # if not user:
    #     HTTPException(404, detail=f"No User with ID '{id}' found")
